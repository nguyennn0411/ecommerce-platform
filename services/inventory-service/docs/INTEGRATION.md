...
## 5. Order Service (Phúc) — cần làm gì?

### 5.1 Cập nhật Feign DTO (bắt buộc để khớp)
...
### 5.2 `OrderItem` + Create Order API
...
## 6. Product Catalog (Thành) — cần làm gì?
...
- [ ] Product API expose `id`, variants (size, color) nếu có
- [ ] Document UUID sản phẩm dùng cho test tích hợp
- [ ] (Tuỳ chọn) Event/webhook khi tạo sản phẩm mới → inventory tạo dòng tồn 0

---

## 7. Test nhanh (Postman / curl)
